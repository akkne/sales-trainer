...
/// bootstrap invite for a brand-new organization's first administrator. See
...
{
    /// <summary>
    /// Every failure raises rather than returning a sentinel, and the three exception types below are
    ...
    /// identity-service could not accept, <see cref="Exceptions
    /// .IdentityOrganizationBootstrapConflictException"/> for an organization that already has an
    /// administrator, and any other exception for everything else — a timeout, an unreachable host, an
    /// unexpected status code — which the caller treats alike as "this call did not complete".
    /// </summary>
