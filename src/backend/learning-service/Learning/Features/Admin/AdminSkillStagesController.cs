...
public sealed class AdminSkillStagesController(LearningDbContext database, ILogger<AdminSkillStagesController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<AdminSkillStageDto>> Create([FromBody] CreateSkillStageRequestDto request)
    {
        ...
        if (await database.SkillStages.AnyAsync(stage => stage.Key == key))
    ...
    public async Task<ActionResult<AdminSkillStageDto>> Update(Guid id, [FromBody] UpdateSkillStageRequestDto request)
    {
        ...
        var stage = await database.SkillStages.FirstOrDefaultAsync(candidate => candidate.Id == id);
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        if (await database.Skills.AnyAsync(skill => skill.Stage == stage.Key))
