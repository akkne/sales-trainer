...
Все запросы идут **через гейтвей** (`http://localhost:5001`), кроме шага 1.
...
# 1.2 Мимо гейтвея, прямо в сервис (порт learning-service) с тем же заголовком.
#     Это НЕ должно быть доступно снаружи вообще: если ответ приходит — сервисные порты
#     торчат наружу, и вся модель изоляции держится только на гейтвее.
curl -H "X-Organization-Id: <id организации B>" http://localhost:5002/admin/assignments
```
