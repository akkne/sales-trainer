...
| `PlatformAdminControllerContractTests` (extended 2026-08-16) | the gate on every admin controller as an attribute-level contract: `/admin/users` reads are `RequirePlatformAdmin` while all three mutations are `RequireSuperAdmin`; `/invites` and `/memberships` are `RequireOrgSuperAdmin` and still `[TenantScoped]`; `/admin/platform/*` is `RequireSuperAdmin` and not tenant-scoped; plus a guard that fails if a new mutating route appears on `AdminUsersController` without being classified |
