...
| `GlobalExceptionHandlerStatusMappingTests` (2026-08-26) | the 4xx/5xx split in `GlobalExceptionHandler`, after a Postgres outage surfaced on the login screen as `400 Bad request`: the `InvalidOperationException` Entity Framework raises for a **transient database failure is a 500**, not a 400, and a 500 **withholds the exception message** (it can carry connection strings and internal host names); `EmailAlreadyRegisteredException` still maps to `400` with `Email already registered.` in `detail` (the register route itself answers `409` — `AuthController` catches it first, covered by `AuthFlowTests.Register_SameAddressTwice_IsRejectedAsConflict`); the 401/403/403/404 mappings the front end branches on are pinned |
