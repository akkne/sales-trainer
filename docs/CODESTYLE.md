...
Revised by the project owner 2026-08-18. Before that date the rule forbade comments
entirely, including `///`; the code never matched it (1216 lint violations against 88 on
`main`), so the gate was meaningless in both directions. The rule now splits the two cases
that were being conflated: **API documentation is required, inline narration is banned.**

### `///` XML documentation — required on services and infrastructure

Every service and infrastructure class carries a `/// <summary>` on the type, and on each
public member whose contract is not obvious from its signature. It states the class's
responsibility and the invariants a caller must respect — not a restatement of the name.

```csharp
/// <summary>
/// Resolves the lesson version a progress row is bound to, creating a frozen version on
/// first use. Never rebinds an existing row: historical accuracy is measured against the
/// version the learner actually answered.
/// </summary>
internal sealed class LessonVersionResolver : ILessonVersionResolver
```

- **Required:** service classes and their interfaces, infrastructure (repositories, clients,
  message handlers, hosted services, filters, middleware), and configuration classes.
- **Not required:** models, entities, DTOs, records, and enums. Their names and property
  names carry the meaning; a `/// <summary>The user's display name.</summary>` on
  `DisplayName` is noise. Document a model only where a non-obvious invariant lives on it
  (a nullable column that means "platform default", not "unlimited").
- Document **why and what is guaranteed**, not **how**. A summary that repeats the method
  name adds a line and no information — omit it.

### `//` explanatory comments — forbidden

If code needs a `//` to be understood, rename the variable or decompose the method. Move
the rationale into the `///` summary or into `docs/DECISIONS.md`.
...
reason, `#pragma`, `<auto-generated>` in generated files).

Enforced by `scripts/codestyle-lint.py`: `///` passes, `//` and `/* */` fail.
