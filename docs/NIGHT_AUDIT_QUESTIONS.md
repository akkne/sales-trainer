...
**Ответ владельца (2026-08-22): не делать. Закрыто.** История до 40.22 остаётся навсегда неполной.
Единственный возможный путь — межсервисная миграция данных (читать реальные сессии из Mongo
ai-service и писать строки прямо в Postgres learning-service, минуя Kafka) — не оправдан ради
горстки предпродовых аккаунтов. Код-фикс, который остановил дальнейшую потерю строк, уже в `main`;
скрипт бэкфилла не писался. Записано в `docs/DECISIONS.md`.
...
**Ответ владельца (2026-08-22): заводить API сейчас. Сделано.**
`GET/PUT /notifications/preferences` в `notification-service` (`NotificationPreferencesController`),
хранилище — один небольшой Redis-хэш на пользователя.

- **Ключ `notifications:preferences:{userId}` без префикса `org:{orgId}:`**, и роут сознательно
  **не** `[TenantScoped]` — в отличие от `NotificationController` рядом. Настройка уведомлений это
  утверждение человека про свой собственный inbox, а личность в этом продукте кросс-организационная
  (`docs/TENANCY/TENANCY.md` §4.2): org-scoped настройка означала бы, что продавец в двух
  организациях выключает напоминания дважды и продолжает получать их от той, на которую не смотрел.
- **Без TTL**, в отличие от 30-дневного inbox: истекающая настройка тихо включила бы «продуктовые
  обновления» обратно — ровно тот отказ, ради предотвращения которого тумблер и существует.
  Удаляется по `user.deleted` рядом с репликой пользователя, чтобы не пережить свою личность и не
  примениться к переиспользованному id.
- **Дефолты** (напоминания включены, продуктовые обновления выключены) объявлены один раз на сервере
  и совпадают со старыми браузерными, поэтому ни у кого фактические настройки не изменились.
- **`isDefault`** отличает «никто никогда не сохранял» от «сохранил и выбрал дефолты». Это то, что
  делает безопасной разовую миграцию старых `localStorage`-значений (пункт (б) выше): фронт
  выгружает локальное значение только пока `isDefault === true`, поэтому устаревший `localStorage`
  на одной машине не может перезаписать настройку, сохранённую с другой. `GET` не создаёт строку, так
  что простое открытие экрана не закрывает путь миграции. При провале выгрузки локальные ключи
  остаются, и следующий визит пробует снова — вместо того чтобы потерять выбор человека навсегда.
- **Честное ограничение (пункт (в) выше остаётся в силе):** настройку пока никто не читает, потому
  что рассыльщика «продуктовых обновлений» или «напоминаний о практике» в сервисе по-прежнему нет —
  единственный мейлер шлёт про непрочитанные чаты. Тумблеры теперь *доступны для чтения* тому
  мейлеру, который появится следующим; это и было целью — они были нечитаемы вообще. Подключать к
  ним существующий чат-мейлер намеренно не стали: он про другое.

Записано в `docs/API_CONTRACTS.md`, `docs/NOTIFICATION_SERVICE.md`, `docs/DECISIONS.md`. Старый стор
`shared/stores/notification-preferences-store.ts` удалён.

---

## Ответы владельца

**Все вопросы отвечены (2026-08-22).** Ответы записаны **внутри блока каждого вопроса**, а не здесь —
чтобы решение стояло рядом с тем, что его вызвало, и его нельзя было прочитать без контекста.
Сводка со ссылками на код — в `docs/DECISIONS.md`, запись «2026-08-22 — owner's answers».

| # | Ответ | Итог |
|---|---|---|
| Q-1 | действие человека | `git push` + redeploy. **Сначала** проверить `INTERNAL_SERVICE_SECRET` на сервере (см. предупреждение в блоке Q-1), потом деплоить. Не сделано — агент не трогает сервер. |
| Q-2 | дочитывать `orgName` | Уже сделано более ранним прогоном. |
| Q-3 | не делать | История `UserDialogScores` до 40.22 остаётся неполной навсегда. |
| Q-4 | завести API | `GET/PUT /notifications/preferences`. Сделано. |
| Q-5 | убрать из навигации | «Leagues»/«Gamification» убраны из админ-навигации. W-15 закрыта. |
| Q-6 | честно сказать на `/tree` | Best-effort сохранён, провал больше не молчит. Отменяет вариант (а), отгруженный ранее. |
| Q-7 | ничего не делать | RLS остаётся выключенным (T-1). T-2 уже закрыт фиксом `1a7606c`. |
| Q-8 | завести bulk-эндпойнт | `PUT /admin/lessons/{lessonId}/exercises/reorder`, одна транзакция. Сделано. |
| Q-9 | действие человека | Перепроверить репро AD-7 на проде после деплоя. |
| Q-10 | ничего не делать | Экран квоты остаётся read-only для чужих организаций. |
| Q-11 | оставить как есть | `stage: "general"` допустим; навык без стадии — нормальное состояние. |
| Q-12 | работа с контентом | UI для назначения навыков технике уже построен (AD-3, `06313d65`); пройти по 45 техникам — человеку. |
| Q-13 | уже закрыто кодом | Разблокировка выводится при чтении (`6893c2ad` + R2-10); бэкфилл не нужен. |
| Q-14 | решение не требовалось | Свип `isError` → `isLoadingError` выполнен. |
| Q-15 | 70 за верную строку | Сделано (`2436d6ed`). |
| Q-16 | вариант 1 — оставить | Пропуск завершает урок при `BestScore = 0`; зафиксировано намеренно. |
...
**Ответ владельца (2026-08-22): убрать из навигации. Сделано, W-15 закрыта.** Обе записи убраны из
навигации админки. Сами роуты и экраны не удалялись — они всё ещё работают, если набрать адрес
руками, — но из панели к необратимой кнопке на снятой с продукта механике больше ничего не ведёт.

Заодно список навигации вынесен из компонента в `features/admin/constants/navigation.ts`
(`PLATFORM_NAVIGATION_ITEMS`), по образцу `ORGANIZATION_NAVIGATION_ITEMS`: теперь это одно
тестируемое значение, причина отсутствия этих двух записей написана там же, где сам список, а тест
`__tests__/platformNavigation.test.ts` замечает, если кто-то вернёт их следующим диффом. Карта иконок
`NAV_ICONS` при этом свернулась в сами элементы.
...
**Ответ владельца (2026-08-22): вариант (б). Сделано.** Важно: между постановкой вопроса и ответом
один из прогонов уже реализовал вариант (а) — убрал `try/catch` и дал провалу утопить всю мутацию.
Ответ владельца это **отменяет**: запись навыков снова best-effort, потому что мигающий бэкенд не
должен запирать нового пользователя на экране онбординга за кнопкой «повторить», на которую тот
никак не влияет. Но она больше не *молчаливая* — а именно это и было содержанием W-13.

- `submitOnboarding()` вынесена из мутации (по образцу `completeDialogSession`), чтобы асимметрия
  двух записей была тестируема без React-дерева: `POST /onboarding` вправе утопить онбординг,
  `PUT /skills/enrolled` — нет, но его провал возвращается как `didPersistSkillSelection: false`,
  а не как тишина.
- `shared/stores/onboarding-skill-selection-store.ts` доносит этот факт до `/tree` — это другой роут.
  Хранится в `localStorage`, а не в памяти: перезагрузка `/tree` иначе потеряла бы факт, а это ровно
  тот исход «человек так и не узнал», из-за которого W-13 и была написана. Гидратация явная
  (в эффекте), чтобы флаг не расходился между серверным и клиентским рендером.
- `/tree` показывает одну строку — что случилось и ссылку на профиль, — закрываемую, и гаснущую
  навсегда при любом успешном `PUT /skills/enrolled` (`useUpdateEnrolledSkills.onSuccess`), то есть
  ровно тогда, когда сообщение перестаёт быть правдой.

Тесты: `__tests__/onboardingSkillSelection.test.ts` (7). Записано в `docs/DECISIONS.md`.
...
**Ответ владельца (2026-08-22).**

**T-1 — RLS остаётся выключенным. Ничего не делать, вообще ничего.** Сервисы продолжают подключаться
под владельцем схемы, `FORCE ROW LEVEL SECURITY` к нему не применяется, и изоляцию держит
исключительно слой EF query filters. Шаг 12 `docs/TENANCY/RUNBOOK.md` сознательно остаётся
невыполненным; включение потребовало бы сперва решить судьбу семи фоновых задач с
`IgnoreQueryFilters()` (отдельная `BYPASSRLS`-роль либо явная отсрочка), а это отдельная работа.
Формулировки в комментариях вида «RLS уже доказал, что вызывающий внутри организации»
(`ContentAuthoringGuard` и др.) **оставлены как есть** — ответ владельца был буквально «ничего не
делать», и правка комментариев в него не входила. Зафиксировано в `docs/DECISIONS.md`, чтобы
следующий читатель кода знал, что расхождение известно и намеренно, а не пропущено.

**T-2 — уже неактуально.** Ночной фикс `1a7606c` пинит `ASPNETCORE_ENVIRONMENT=Production` во всех
сервисах прод-оверлея, а внутренний service-to-service фильтр стал fail-closed. Предупреждение из
Q-1 остаётся в силе и остаётся действием человека: **сначала** убедиться, что
`INTERNAL_SERVICE_SECRET` выставлен на сервере, **потом** пушить и деплоить.
...
**Ответ владельца (2026-08-22): заводить bulk-эндпойнт. Сделано.**
`PUT /admin/lessons/{lessonId}/exercises/reorder`, тело `{exercises: [{exerciseId, orderInLesson}]}`,
ответ — весь список в новом порядке. Действие целиком выполняется внутри одной write-транзакции,
которую контроллер уже открывает через `[TenantTransaction]`, поэтому единственный
`SaveChangesAsync` — это та самая атомарность, которой у пачки отдельных `PUT` быть не могло: при
падении на третьей строке из четырёх урок оставался с двумя упражнениями на одной позиции и без
записи о том, чего вообще хотел оператор (R2-2).

Валидация — вся до записи, любая ошибка это 400 и ноль изменений:
- запрос обязан назвать **каждое** упражнение урока, а не только переставленные. Подмножество
  невозможно проверить на коллизии с теми строками, которые оно не упоминает, — принимая его, мы
  вернули бы через дырку в валидации ровно те дубли позиций, ради которых роут и заводится;
- один `exerciseId` дважды, две строки на одной позиции, id из другого урока, несовпадение
  количества — всё отказ;
- позиции обязаны быть **различными**, но намеренно не обязаны быть непрерывными или начинаться с
  нуля: чтение сортирует по столбцу, а не индексирует по нему, и существующие уроки несут ту
  нумерацию, которую им дал их импорт.

Все три редактора упражнений (два `/admin/**` экрана и org-редактор оверрайдов) теперь шлют один
запрос вместо цикла. Админские экраны снова могут откатывать локальный порядок при ошибке: раз
запрос атомарный, на сервере ничего не сдвинулось, и откат это правда, а не выдуманное состояние.

`PUT /admin/exercises/{id}` не тронут и по-прежнему принимает `orderInLesson` — он остаётся способом
править тело одного упражнения. Тесты: `AdminExercisesReorderTests` (10). Записано в
`docs/API_CONTRACTS.md` и `docs/DECISIONS.md`.
...
**Ответ владельца (2026-08-22): вариант 3 — ничего не делать. Закрыто.** Экран остаётся read-only для
чужих организаций (чтение уже починено выше и показывает цифры именно той организации, что в URL).
Ни путь 1 (пустить impersonation в этот эндпойнт + узкое исключение в `(admin)/layout.tsx`), ни путь
2 (прямая кросс-тенантная запись для platform staff) не стоят своей поверхности безопасности ради
одного экрана: изменение квоты клиенту — операция по тикету, разовым скриптом, а не кнопкой в
админке. Ничего небезопасного в текущем состоянии нет — сохранить в чужую организацию экран уже
отказывается, а не делает это молча. Записано в `docs/DECISIONS.md`.
...
**Ответ владельца (2026-08-22): оставить как есть. Закрыто, без правки данных и без 6-й стадии.**
`pipeline-management` сохраняет `stage: "general"`. Ночной фикс отображения уже сделал так, что
оператору ничто не врёт: неизвестная стадия рендерится как «Другое», а в форме редактирования есть
явный вариант «— не назначена (general) —» вместо молчаливого предвыбора «Подготовка». Этого
достаточно.

И это же ответ на вторую половину вопроса — про общую политику: **навык без назначенной стадии
воронки это допустимое состояние**, а не дефект данных, который надо чинить каждый раз. Отдельная
строка «unassigned» в реестре стадий не нужна: реестр описывает пять стадий воронки, а «вне одной
стадии» это не шестая стадия, а отсутствие ответа, и фронт уже отображает его честно. Записано в
`docs/DECISIONS.md`.
...
**Статус (2026-08-22): развилка 1 уже закрыта кодом, развилка 2 стала неактуальной.** Вопрос владельцу
не задавался, потому что отвечать на него уже не нужно — проверено чтением кода, а не по чужим
словам:

Выбран (и уже реализован) первый вариант — пересчёт доступности при чтении. `6893c2ad` («fix: derive
lesson unlock from completion facts, not the unlock-transition event») плюс R2-10 сделали так, что
`ExerciseService.GetLessonsForSkillAsync` и `GetLessonsForTopicAsync` идут по урокам навыка в порядке
прохождения и открывают любой `Locked`-или-отсутствующий урок сразу после завершённого — на каждом
чтении, без опоры на событие перехода. Сохранённый статус `Available`/`InProgress`/`Completed` всегда
побеждает выведенный, поэтому механизм умеет только открывать и никогда не двигает урок назад.

Отсюда развилка 2 отпадает сама: аккаунт, закрывший тему до появления цепочки разблокировки,
**самоисцеляется на первом же чтении** после деплоя. Ни разовой миграции, ни правки
`UserLessonProgress` руками на живой базе не требуется. Единственное, что осталось — задеплоить (это
Q-1).
