...
- [~] Уведомление РОПу о новом оспаривании не отправляется: администраторов организации в
      платформе сейчас нечем перечислить (`/internal/memberships/active` отдаёт id без ролей).
      Оспаривание видно в очереди дашборда; пуш логически принадлежит 40.26, где решается тот
      же вопрос про адресацию РОПу. `docs/DONT_FORGET.md`
