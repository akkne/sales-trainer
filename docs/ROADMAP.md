...
### [x] 40.20 Разделение админки
- [x] Платформенная суперадминка: организации, глобальная библиотека, типы упражнений —
      осталась на месте целиком (14 экранов, английский), плюс одно добавление:
      `/admin/organizations/[id]/quota`
- [x] Админка организации (экран РОПа): своя программа, override'ы, профиль компании,
      люди, задания — **второе дерево `app/(org)/`**, 19 экранов + состояние O0, по-русски
- [x] Ревизия всех текущих `/admin/*` маршрутов: каждый относится к одному из двух уровней
      — итог в `docs/TENANCY/ADMIN_UI_DESIGN.md` §3.4 и в таблице политик `docs/ADMIN_PANEL.md`
- [x] Обновить `docs/ADMIN_PANEL.md` — раздел «Two panels (40.20)», дерево `/org/*`, таблица
      редиректов, правила оформления обеих панелей; заодно исправлены две устаревшие строки
      (`RequireOrgAdmin` «без вызовов» и `PUT /organizations/profile` «открыт любому участнику»
      в `docs/API_CONTRACTS.md`)

> **Дизайн получен и записан (2026-08-18): `docs/TENANCY/ADMIN_UI_DESIGN.md`.** Реализация
> разбита на двенадцать непересекающихся слайсов (§5). **Слайс 0 — каркас — построен:**
> дерево `app/(org)/`, гейт на обеих осях, состояние O0, сайдбар с девятью пунктами и тремя
> бейджами, таблица редиректов для двух ссылок из уведомлений 40.26, семь новых общих
> компонентов, два предиката в `auth-store`, пункт «Управление» в `nav-rail`. Экраны O1–O19 —
> слайсы 1–11. Проверки: `docs/TESTING/ORG_PANEL.md`.

> **Блок закрыт 2026-08-19.** Все двенадцать слайсов построены: каркас + O1–O19 + экран квот в
> платформенной панели. Фронт — **936 тестов** (было 353 на приёмке фазы 40), `tsc`, eslint и
> `next build` зелёные; бэкенд — **1001 тест**, сборка без ошибок. Разделение сделано **вторым
> деревом, а не миграцией первого**: ни один существующий экран не переехал, поэтому одиннадцать
> агентов писали параллельно, не сталкиваясь.
>
> Попутно закрыты четыре бэкенд-нехватки, найденные при постройке экранов (не при чтении
> документации): `GET /memberships`, `GET /invites`, `orgName` в `GET /auth/me` и корневой маршрут
> `/memberships` в гейтвее. Что осталось владельцу и что каждый экран не смог получить от бэкенда —
> в `docs/DONT_FORGET.md`, раздел «Блок 40.20», и в «чего не может бэкенд» каждой секции
> `docs/TESTING/ORG_PANEL.md`.

> **Историческая справка — «ждём дизайн от владельца» (2026-08-16).** Ролевая модель и авторизация для обоих уровней
...
### [x] 40.37 Открытая регистрация и гейт «нет организации»
> Пересматривает 40.7, который удалил `POST /auth/register`. Владелец вернул форму
> регистрации 2026-08-20; `docs/RAW.md` требовал её с самого начала («регистрация через
> email или Google»). Ключ к тому, что это не откат tenancy-границы: 40.7 закрывал
> **аккаунт**, чтобы закрыть **доступ к организации**, а это две разные вещи.
- [x] `POST /auth/register` создаёт `user` и **не создаёт** `membership` — ветки, которая
      могла бы его создать, в методе нет. Без активного членства в JWT нет `org_id`, значит
      RLS и tenant-scoped запросы не видят ничего
- [x] `409` на занятый адрес — единственный маршрут сервиса, где уклончивый ответ невозможен:
      форма, не признающая, что адрес занят, не может и создать аккаунт. Уклончивые
      формулировки `/auth/login`, `/auth/login/start`, `/auth/google` сохранены (они утекают
      **организацию**, чего регистрация не делает)
- [x] `POST /auth/google` снова провижинит: неизвестная Google-личность становится аккаунтом
      без членства. Проверка активного членства снята — она отвергала то, что вход по паролю
      пускал. Осталось одно отклонение: адрес занят **неверифицированной** локальной строкой
- [x] Верификация почты вынесена в флаг `EmailVerification:Enabled`
      (`EMAIL_VERIFICATION_ENABLED`), **выключен**. Один переключатель на оба конца правила:
      включённый — регистрация шлёт код и не выдаёт сессию (`202`), а вход отвергает
      неподтверждённый аккаунт (`403`); выключенный — адрес верифицируется сразу
- [x] Фронт: страница `/register`, ссылка с `/login`, хук `useRegister` на оба исхода,
      `register` в белом списке аналитики (заодно добавлен потерянный с 40.7 `invite`)
- [x] Гейт `AwaitingOrganizationGate` перед всем `app/(main)`: пользователь без организации
      видит экран ожидания приглашения с выходом и ссылкой в поддержку. Платформенный
      персонал проходит (роли Sellevate не привязаны к tenancy), неаутентифицированный
      посетитель тоже — гейт не должен стать вторым редиректом на логин
- [x] Тесты: 132 юнита identity (провижининг без членства, оба положения флага, отказ по
      неверифицированной строке), 6 фронтовых на гейт, 3 интеграционных на `/auth/register`
- [x] Обновить `TENANCY.md` §4.1a, `API_CONTRACTS.md`, `EMAIL_VERIFICATION.md`,
      `IDENTITY_SERVICE.md`, `TESTING/REGISTRATION.md`
