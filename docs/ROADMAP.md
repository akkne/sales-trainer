...
- [ ] Hangfire recurring job (runs daily at 00:05): for each user check `LastActivityDate`
  - if yesterday → streak continues (no-op, updated on exercise submit)
  - if > 1 day ago → reset `CurrentStreakDayCount` to 0
- [ ] Update `ExerciseService` to set `LastActivityDate = today` and increment streak on submit
- [ ] Expose `currentStreak` in Profile API response
- [ ] Show streak on profile page

### [ ] XP source tracking
- [ ] `UserXp` records created on exercise completion (source: `exercise`)
- [ ] Streak bonus XP on 7-day and 30-day milestones (source: `streak_bonus`)
- [ ] Weekly XP totals fed into league standings
