...
### [x] W-11 Редактор урока организации: подтверждение удаления упражнения закрывается независимо от результата
...
### [x] W-15 Админка лиг: четыре мутации без единого сообщения об ошибке (устаревший раздел, но раздел живой)
...
- **Ущерб:** нет сообщения
- **Severity:** minor
- **Итог:** `adjustXp`, `removeMembership`, `resync`, `moveTier` и `closeWeek` уже получили
  `toast.error` в рамках W-6 (`b393059`, все 48 `onError` в `use-admin.ts`) — «уходят только в
  clientLogger» больше не так. Отдельно для «закрыть неделю» (`admin/leagues/page.tsx`) добавлено
  явное сообщение на странице — `closeWeek.isError` рендерит «Failed to close the week: … Nothing
  was closed», рядом с кнопкой — поверх toast, потому что действие необратимо и провал не должен
  зависеть от того, успел ли админ заметить toast за 6 секунд. Кнопка и подтверждение
  (`onSettled`) не трогал: раздел решил не трогать по существу (Q-5), а после провала кнопка
  возвращается в обычное состояние «Close current week», а не в состояние, похожее на успех —
  успеха на экране никак не показывается вообще, так что провал и успех не путаются местами.
  Раздел из навигации не убирал — решение по Q-5 за владельцем.
