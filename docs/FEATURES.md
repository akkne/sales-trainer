...
| [Tenancy — assignments & AI admin](TENANCY/ASSIGNMENTS.md) | **§1 and §1.1 built (Phases 40.21–40.22), the rest design** — the post-training practice loop (РОП → managers). Built: the `Assignment` entity and `assignment_progress`, both strict tenant data, with content stored as references to frozen lesson versions (so the eleven existing exercise types render an assignment with no new code), an audience *rule* rather than a resolved list, and a freeze trigger on issue; plus the completion-rule vocabulary (`dialog_score`, `exercise_accuracy`) and its evaluation, which reuses the existing scoring, keeps `failed_threshold` a visible state, and derives every number from attempt rows so a redelivered event cannot inflate it. Still design: audience fan-out and notifications (40.23 — nothing creates a progress row yet, so the funnel still reads zero), auto-repeats (40.24), the РОП dashboard (40.25), the non-completion push (40.26), and the six ways AI removes admin work (Stage F) |
