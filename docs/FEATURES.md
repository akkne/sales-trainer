...
| [Tenancy — content model](TENANCY/CONTENT_MODEL.md) | **Partly built (40.15)** — immutable lesson versioning is implemented (`Lessons` + `LessonVersions`, canonical-JSON `content_hash`, one draft per lesson, freeze trigger); the rest is still design — why not to fork the curriculum per customer; immutable lesson versioning (`lesson` / `lesson_version` / programme pinning), copy-on-write overrides + stale review, and the organization profile that removes most forks by substitution |
