...
| [Deployment](DEPLOYMENT.md) | Production deploy: self-hosted frontend (`sellevate.site`) + backend + infra all via Docker Compose behind Traefik, CORS allow-list, env vars, optional Kubernetes/Helm (Option C) + health probes |
...
| [Redesign Prompt](REDESIGN_PROMPT.md) | **Current (V4, 2026-08-26)** — ready-to-paste Claude Design brief: product context, the three interface zones and their audiences, what is fixed (the lime mark and its fill-only physics, both themes, the Russian labels, no gamification) versus free, every screen including the `/org` half, required states and deliverables. Rewritten from scratch; the earlier brief described the pre-2026-08 product (league, hearts, XP, green consumer look) |
...
| [NIGHT_AUDIT_ANSWERS.md](TESTING/NIGHT_AUDIT_ANSWERS.md) | The four things built once the owner answered the open `NIGHT_AUDIT_QUESTIONS.md` blocks (2026-08-22): the notification-preferences API (Q-4), the unlinked gamification screens (Q-5), onboarding's honest `/tree` line (Q-6) and the atomic exercise-reorder endpoint (Q-8) — with the failure paths (forced 500s) that are the actual point of three of them |
