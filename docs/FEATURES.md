...
| [Tenancy — assignments & AI admin](TENANCY/ASSIGNMENTS.md) | **§1 built (Phase 40.21), the rest design** — the post-training practice loop (РОП → managers). Built: the `Assignment` entity and `assignment_progress`, both strict tenant data, with a required quality-threshold `completion_rule` that has no default, content stored as references to frozen lesson versions (so the eleven existing exercise types render an assignment with no new code), an audience *rule* rather than a resolved list, and a freeze trigger on issue. Still design: threshold evaluation (40.22), audience fan-out and notifications (40.23), auto-repeats (40.24), the РОП dashboard (40.25), the non-completion push (40.26), and the six ways AI removes admin work (Stage F) |
