...
### [x] T-1 RLS выключен во всех развёрнутых окружениях — рантайм ходит под суперпользователем

**Проверено (2026-08-21, ночной прогон) — репо-часть уже была готова, ничего не пришлось**
**добавлять.** Создание роли на реальном сервере — не моё (правило прогона), поэтому шаг 12
**не** выполнен и не отмечается как выполненный. Но всё, что относится к «репо готово + гэп
громко задокументирован», уже сделано в Phase 40.4/40.10–40.18 и я перепроверил, что это верно
и полно, а не только заявлено:

- `docker-compose.yml` — все семь Postgres-сервисов (identity, ai, learning, company,
  gamification, social, organization) собирают `ConnectionStrings__Postgres` из
  `${APP_POSTGRES_USER:-$POSTGRES_USER}` / `${APP_POSTGRES_PASSWORD:-$POSTGRES_PASSWORD}`, а
  `ConnectionStrings__PostgresMigrations` — всегда из `${POSTGRES_USER}` (владелец). Пока
  `APP_POSTGRES_*` не задан — поведение ровно сегодняшнее (владелец = и рантайм, и DDL);
  выставить их — это правка `.env`, не кода.
- `.env.example:16-22` — `APP_POSTGRES_USER`/`PASSWORD` закомментированы со ссылкой на
  `create_sellevate_app_role.sql` и на шаг 12 (включая явное предупреждение про семь фоновых
  BYPASSRLS-задач), т.е. дефолт безопасен и гэп виден при первом чтении файла.
- `docs/TENANCY/sql/create_sellevate_app_role.sql` — создаёт `sellevate_app`
  (`NOSUPERUSER`/`NOCREATEDB`/`NOCREATEROLE`/`NOBYPASSRLS`), не запускался ни против чего, кроме
  одноразового тестового контейнера (комментарий в файле это прямо говорит).
- `docs/TENANCY/RUNBOOK.md` шаг 12 — по шагам: создать роль (SQL выше), выставить `.env`, три
  предостережения (семь фоновых задач замолчат без `BYPASSRLS`, миграции теперь по отдельной
  строке, прогнать `TestCategory=Integration`).
- `docs/DONT_FORGET.md` («База данных и RLS», ~строка 108) — тот же гэп человеку, плюс отдельные
  блоки на каждый сервис (learning/ai/company/gamification/social) про свои BYPASSRLS-джобы.
- Владельцу уже задан явный вопрос с двумя вариантами (выполнять шаг 12 сейчас, включая судьбу
  семи BYPASSRLS-задач, или сознательно оставить RLS выключенным и убрать из комментариев вида
  `ContentAuthoringGuard.MayAuthor` формулировки «RLS уже доказал…», которые сейчас описывают не
  то, что происходит) — `docs/NIGHT_AUDIT_QUESTIONS.md` Q-7.

Ничего не менял в компоузе/скриптах/RUNBOOK — было бы дублированием уже существующей и верной
работы. Шаг 12 остаётся невыполненным по факту, это не скрывается.
