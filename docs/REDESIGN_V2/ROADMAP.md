...
- [ ] **P4 — Screen restyles** (each sub-phase = its own CSS + commit + verifier)
  - [x] 4.1 Path / `tree` ✅ 3-col accordion + timeline + FAB + overview; tsc clean, vitest 78/78. Omitted "what you'll learn"/related-techniques (no backend data). Lesson task-type chips collapsed to a single Теория/Практика chip — `LessonSummary.kind` carries only theory/practice, not per-exercise types, so the granular chipMap is not surfaced on lesson cards (data-contract omission).
    - 2026-07-25 usability pass: accuracy stat now computed as avg `bestScore` over completed lessons (dead "Время" cell replaced with "Осталось"); per-lesson score chip on completed cards; lock icon on locked nodes; overall path progress bar in left column + mobile sheet; auto-select of in-progress skill + stale-selection recovery; timeline auto-scrolls to current lesson; FAB hides via IntersectionObserver when the active card is on screen; right "Обзор" panel now shows next-lesson quick-start and last activity (XP/streak block was added then removed — gamification is out of the app); mobile stat grid 2×2.
